    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        >
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900">Logotipo da Marca</h3>
                    <p class="text-sm text-gray-500" v-if="!sections.logo">{{ project.logo_path ? 'Logo enviado' : 'Defina a identidade visual' }}</p>
                </div>
            </div>
            <div class="text-gray-400 transition-transform duration-300" :class="{'rotate-180': sections.logo}">
        ...
        <div v-show="sections.logo" class="p-6 border-t border-gray-100 bg-gray-50/50 transition-all duration-300 ease-in-out">
            <div class="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-xl p-8 hover:border-blue-500 hover:bg-blue-50 transition-all cursor-pointer relative group bg-white">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        >
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900">Galeria de Fotos</h3>
                    <p class="text-sm text-gray-500" v-if="!sections.photos">{{ (project.context_photos?.length || 0) + ' fotos enviadas' }}</p>
                </div>
            </div>
            <div class="text-gray-400 transition-transform duration-300" :class="{'rotate-180': sections.photos}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
            </div>
        </div>

        <div v-show="sections.photos" class="p-6 border-t border-gray-100 bg-gray-50/50 transition-all duration-300 ease-in-out">
            <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-purple-500 hover:bg-purple-50 transition-all cursor-pointer relative group h-48 flex flex-col justify-center items-center bg-white mb-6">
            ...
            <!-- Preview Grid -->
            <div v-if="project.context_photos && project.context_photos.length > 0">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4">
                    <div v-for="(photo, index) in project.context_photos" :key="index" class="relative group aspect-square rounded-lg overflow-hidden border border-gray-200 shadow-sm bg-white">
